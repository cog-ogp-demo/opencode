...
describe("run runtime queue", () => {
  ...
  test("treats /cost as a local command", async () => {
    const ui = footer()
    const seen: string[] = []
    let costCalled = 0
    ...
    expect(costCalled).toBe(1)
    expect(seen).toEqual(["hello"])
  })

  test("shell mode submits /cost as a shell command", async () => {
    const ui = footer()
    ...
    ui.submit("/cost", "shell")
    await task

    expect(costCalled).toBe(0)
    expect(seen).toEqual([{ text: "/cost", parts: [], mode: "shell" }])
  })

  test("/cost is not exposed as a managed queued prompt", async () => {
    const ui = footer()
    let costCalled = 0
    const queuedEvents: FooterEvent[] = []
    ...
    const task = runPromptQueue({
      ...
      run: async () => {
        // Submit /cost while a turn is active — it should bypass managed queueing
        ui.submit("/cost")
    ...
    expect(costCalled).toBe(1)
    // /cost should never appear in queued.prompts events
    for (const event of queuedEvents) {
      if (event.type !== "queued.prompts") continue
      expect(event.prompts.some((p) => p.prompt.text === "/cost")).toBe(false)
    }
  })
